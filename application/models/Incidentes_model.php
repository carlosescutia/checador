...
class Incidentes_model extends CI_Model {
    ...
    public function get_lista_incidentes_empleados_todos($mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        ...
        $sql = 'select e.cve_empleado, e.cod_empleado, e.nom_empleado, h.desc_horario, count(j.cve_incidente) as num_incidentes from empleados e left join justificacion_periodo(?,?,?,?) j on j.cve_empleado = e.cve_empleado and j.cve_incidente is not null and j.tipo_justificante is null left join horarios h on e.cve_horario = h.cve_horario where e.activo = 1 group by e.cve_empleado, e.cod_empleado, e.nom_empleado, h.desc_horario order by e.nom_empleado';
    ...
    public function get_lista_incidentes_empleados_pendientes($mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = 'select j.cve_empleado, e.cod_empleado, e.nom_empleado, h.desc_horario, count(j.*) as num_incidentes from justificacion_periodo(?,?,?,?) j left join empleados e on j.cve_empleado = e.cve_empleado left join horarios h on e.cve_horario = h.cve_horario where j.cve_incidente is not null and j.tipo_justificante is null group by j.cve_empleado, e.cod_empleado, e.nom_empleado, h.desc_horario order by e.nom_empleado';
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia));
        return $query->result_array();
    }
    ...
    public function get_incidentes_empleado($cve_empleado, $mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        ...
        $sql = ""
            ...
            ."j.fecha "
            ;
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia, $cve_empleado));
        return $query->result_array();
    }
    ...
    public function get_lista_incidentes_fechas($mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = 'select j.fecha, count(j.cve_incidente) as num_incidentes from justificacion_periodo(?,?,?,?) j where j.cve_incidente is not null and j.tipo_justificante is null group by j.fecha order by j.fecha';
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia));
        return $query->result_array();
    }
    ...
    public function get_incidentes_fecha($fecha, $mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = 'select j.cve_empleado, e.cod_empleado, e.nom_empleado, h.desc_horario, count(j.*) as num_incidentes from justificacion_periodo(?,?,?,?) j left join empleados e on j.cve_empleado = e.cve_empleado left join horarios h on e.cve_horario = h.cve_horario where fecha = ? and j.cve_incidente is not null and j.tipo_justificante is null group by j.cve_empleado, e.cod_empleado, e.nom_empleado, h.desc_horario order by e.nom_empleado';
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia, $fecha));
        return $query->result_array();
    }
    ...
    public function get_tot_incidentes($mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = 'select count(j.cve_incidente) as tot_incidentes from justificacion_periodo(?,?,?,?) j where j.cve_incidente is not null and j.tipo_justificante is null' ;
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia));
    ...
    public function get_tot_empleados_incidentes($mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = 'select count(distinct j.cve_empleado) as tot_empleados_incidentes from justificacion_periodo(?,?,?,?) j where j.cve_incidente is not null and j.tipo_justificante is null ' ;
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia));
    ...
    public function get_tot_dias_incidentes($mes, $anio, $tolerancia_retardo, $tolerancia_asistencia)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = 'select count(distinct j.fecha) as tot_dias_incidentes from justificacion_periodo(?,?,?,?) j where j.cve_incidente is not null and j.tipo_justificante is null' ;
        $query = $this->db->query($sql, array($fech_ini, $fech_fin, $tolerancia_retardo, $tolerancia_asistencia));
    ...
    public function get_tot_dias_habiles($mes, $anio)
    {
        $fech_ini = $anio . "-" . $mes . "-01";
        $fech_fin = date("Y-m-t", strtotime($fech_ini));
        $sql = "select count(*) as tot_dias_habiles from dias_habiles_periodo(?,?) ";
        $query = $this->db->query($sql, array($fech_ini, $fech_fin));
