...
class Horarios extends CI_Controller {
    ...
    public function index()
    {
        if ($this->session->userdata('logueado')) {
            ...
            $this->load->view('catalogos/horarios/lista', $data);
    ...
    public function detalle($cve_horario)
    {
        if ($this->session->userdata('logueado')) {
            $data = [];
            $data += $this->get_userdata();
            ...
            $data['horario'] = $this->horarios_model->get_horario($cve_horario);

            $this->load->view('templates/admheader', $data);
            $this->load->view('catalogos/horarios/detalle', $data);
            $this->load->view('templates/footer', $data);
        } else {
            redirect('admin/login');
        }
    ...
    public function guardar($cve_horario=null)
    {
        if ($this->session->userdata('logueado')) {

            $nuevo_horario = is_null($cve_horario);
            ...
            if ($horario) {
                ...
                $cve_horario = $this->horarios_model->guardar($data, $cve_horario);
				...
				$entidad = 'horarios';
                $valor = $cve_horario . " " . $horario['desc_horario'];
				$data = array(
					'fecha' => date("Y-m-d"),
					'hora' => date("H:i"),
    ...
    public function eliminar($cve_horario)
    {
        if ($this->session->userdata('logueado')) {
            ...
            // registro en bitacora
            $horario = $this->horarios_model->get_horario($cve_horario);
            $separador = ' -> ';
            $usuario = $this->session->userdata('usuario');
            $nom_usuario = $this->session->userdata('nom_usuario');
            $nom_organizacion = $this->session->userdata('nom_organizacion');
            ...
            // eliminado
            $this->horarios_model->eliminar($cve_horario);
